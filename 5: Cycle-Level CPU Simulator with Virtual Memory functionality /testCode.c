...
int GetLD_TEMP_PSR(int *x)         { return(x[LD_TEMP_PSR]); }
int GetGate_PTEAddress(int *x)         { return(x[Gate_PTEAddress]); }
int GetGate_TempPSR(int *x)         { return(x[Gate_TempPSR]); }
int GetMDRInMUX(int *x)         { return((x[MDRInMUX1] << 1) + x[MDRInMUX0]); }
int GetMDROutMUX(int *x)         { return(x[MDROutMUX]); }
...
void eval_micro_sequencer() {
    ...
    int EXC1 = (CURRENT_LATCHES.VA & 0x1) && CURRENT_LATCHES.SavedDATA_SIZE;
    int EXC0 = !EXC1 && ((!(OPCODE == 15) && CURRENT_LATCHES.Priv && !(CURRENT_LATCHES.MDR & 0x8)) || (OPCODE == 10 || OPCODE == 11));
    int EXC2 = !EXC1 && !(OPCODE == 10 || OPCODE == 11) && ((!(OPCODE == 15) && CURRENT_LATCHES.Priv && !(CURRENT_LATCHES.MDR & 0x8)) || !(CURRENT_LATCHES.MDR & 0x4));
    EXC = (EXC2 << 2) + (EXC1 << 1) + EXC0;
    if (GetIRD(CMI)) {
        NEXT_LATCHES.STATE_NUMBER = OPCODE;
    }
    else if (GetRETURN(CMI)) {
        NEXT_LATCHES.STATE_NUMBER = CURRENT_LATCHES.SavedJBits;
    }
    else if (((GetCOND(CMI) == 6) && EXC1) || ((GetCOND(CMI) == 7) && EXC2))  {
        NEXT_LATCHES.STATE_NUMBER = 63;
    }
    else if (GetTRANSLATE(CMI)) {
        NEXT_LATCHES.STATE_NUMBER = 56 + (GetR_W(CMI) ? 5 : 0);
    }
    else {
...
int MARMUX_BUS, PC_BUS, ALU_BUS, SHF_BUS, MDR_BUS, PC_2_BUS, PSR_BUS, SP_BUS, Vector_BUS, PTEAddress_BUS, TempPSR_BUS; // Values to be driven on bus
...
void eval_bus_drivers() {
    ...
    if (GetMDROutMUX(CMI)) {
        MDR_BUS = (CURRENT_LATCHES.MDR & 0x3E00) + (CURRENT_LATCHES.VA & 0x01FF);
    }
    else {
        if (GetDATA_SIZE(CMI)) {
    ...
    PTEAddress_BUS = CURRENT_LATCHES.PTBR + (bitsToDec(CURRENT_LATCHES.VA, 15, 9, UNSIGNED) << 1);
    TempPSR_BUS = CURRENT_LATCHES.TEMP_PSR;
}
...
void drive_bus() {
    ...
    else if (GetGate_PTEAddress(CMI)) {
        BUS = PTEAddress_BUS;
    }
    else if (GetGate_TempPSR(CMI)) {
        BUS = TempPSR_BUS;
    }
    else {
        BUS = 0;
...
void latch_datapath_values() {
    ...
    if (GetLD_MDR(CMI) && GetMIO_EN(CMI)) {
        ...
        int BUS_Output = (CURRENT_LATCHES.MAR & 1) ? (bitsToDec(BUS, 7, 0, UNSIGNED) << 8) + bitsToDec(BUS, 7, 0, UNSIGNED) : BUS;
        switch (GetMDRInMUX(CMI)) {
            case 0:
                NEXT_LATCHES.MDR = BUS_Output;
                break;
            case 1:
                NEXT_LATCHES.MDR = BUS_Output | 0x1;
                break;
            case 2:
                NEXT_LATCHES.MDR = BUS_Output | 0x3;
        }
    }
    ...
    if (GetLD_TEMP_PSR(CMI)) {
        NEXT_LATCHES.TEMP_PSR = BUS;
    }
    if (GetTRANSLATE(CMI)) {
        NEXT_LATCHES.SavedJBits = GetJ(CMI);
        NEXT_LATCHES.SavedDATA_SIZE = GetDATA_SIZE(CMI);
    }
}