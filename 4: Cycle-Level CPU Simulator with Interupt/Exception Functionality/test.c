...
void eval_micro_sequencer() {
    ...
    int ird  = GetIRD(u);      // 0..3
    int cond = GetCOND(u);
    int j    = GetJ(u);

    /* Timer interrupt: set once at cycle 300, clear when ISR entry finishes (don’t clear by time). */
    if (CYCLE_COUNT == 299) NEXT_LATCHES.INTV = 0x01;
    // Example clear point (change 47 to your “ISR entry done” state):
    if (CURRENT_LATCHES.STATE_NUMBER == 47) NEXT_LATCHES.INTV = 0x00;

    /* Exception / unknown detection with proper gating */
    int prot = 0, unal = 0, unk = 0;
    if (cond == 6) {
        unal = GetDATA_SIZE(u) && (CURRENT_LATCHES.MAR & 1);
        prot = (CURRENT_LATCHES.Priv == 1) && (CURRENT_LATCHES.MAR <= 0x2FFF);
    }
    if (ird == 1) { // decode mode
        unk = (OPCODE == 0xA) || (OPCODE == 0xB);
    }
    ...
    if (ird == 1) {                    // Decode opcode
        if (unk) {
            NEXT_LATCHES.STATE_NUMBER = 63;
        } else {
            NEXT_LATCHES.STATE_NUMBER = (CURRENT_LATCHES.IR >> 12) & 0xF;
        }
    } else if (ird == 2) {             // Restore via STATE latch
        if (prot || unal) {
            NEXT_LATCHES.STATE_NUMBER = 63;
        } else {
            switch (CURRENT_LATCHES.STATE) {
                case 2:  NEXT_LATCHES.STATE_NUMBER = 29; break;
                case 3:  NEXT_LATCHES.STATE_NUMBER = 24; break;
                case 6:  NEXT_LATCHES.STATE_NUMBER = 25; break;
                case 7:  NEXT_LATCHES.STATE_NUMBER = 23; break;
                case 18:
                case 19: NEXT_LATCHES.STATE_NUMBER = 33; break;
                default: NEXT_LATCHES.STATE_NUMBER = 0;  break;
            }
        }
    } else {                           // ird==0 or 3: normal sequencing
        if (cond == 6 && (prot || unal)) {
            NEXT_LATCHES.STATE_NUMBER = 63;
        } else {
            NEXT_LATCHES.STATE_NUMBER = j
              | (((cond == 5) && CURRENT_LATCHES.INTV)  << 4)  // interrupt test
              | (((cond == 4) && CURRENT_LATCHES.Priv)  << 3)  // mode test
              | (((cond == 2) && CURRENT_LATCHES.BEN)   << 2)
              | (((cond == 1) && CURRENT_LATCHES.READY) << 1)
              |  ((cond == 3) && (CURRENT_LATCHES.IR & 0x0800) ? 1 : 0);
        }
    }

    /* Latches controlled by control bits (do NOT force state 63 early) */
    if (GetLD_Vector(u)) {
        NEXT_LATCHES.Vector = GetVectorMUX(u) ? CURRENT_LATCHES.EXCV : CURRENT_LATCHES.INTV;
    }
    if (GetLD_Priv(u))     NEXT_LATCHES.Priv = 0;  // entering supervisor
    if (GetLD_USP(u) && !CURRENT_LATCHES.Priv) NEXT_LATCHES.Saved_USP = CURRENT_LATCHES.REGS[6];
    if (GetLD_SSP(u) &&  CURRENT_LATCHES.Priv) NEXT_LATCHES.Saved_SSP = CURRENT_LATCHES.REGS[6];

    /* Record resume state if microcode wants it (prefer to key off LD_STATE instead of a hardcoded list) */
    if (GetLD_STATE(u)) NEXT_LATCHES.STATE = CURRENT_LATCHES.STATE_NUMBER;
